  ...
  if ("sourceIndex" in document.documentElement) {
    ...
    if (document.createRange) {
      sortOrder = function (a, b) {
        ...
        var ret = aRange.compareBoundaryPoints(document.Range.START_TO_END, bRange);
        if (ret === 0) {
          hasDuplicate = true;
        }
        return ret;
...
Drupal.dreditor.patchReview.behaviors.setup = function (context, code) {
  ...
  var ln1 = '';
  var ln2 = '';
  var buildFileMenuLinks = function(full, match1, match2, match3) {
    "use strict";
  ...
  var buildHunkMenuLinks = function(full, match1, match2) {
    "use strict";
    var id = match2.replace(/[^A-Za-z_-]/g, '');
  ...
  for (var n in code) {
    ...
    line = line.replace(/^(\+\+\+ )([^\s]+)(\s.*)?/, buildFileMenuLinks);
    // Build hunk menu links for file.
    line = line.replace(/^(@@ .+ @@\s+)([^\s]+\s[^\s\(]*)/, buildHunkMenuLinks);
 ...
 */
Drupal.behaviors.dreditorIssueSummary = function () {
